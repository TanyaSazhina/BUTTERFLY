<?php 
    ...
    $cat_arr = mysqli_query($link, "SELECT * FROM `categories` WHERE `id` =" . $id);
    $imgs = mysqli_query($link, "SELECT * FROM `slider` WHERE `id_cats` = " . $id );
    $member = mysqli_fetch_assoc($cat_arr);
    $next_id = $member['id'] + 1;
    $prev_id = $member['id'] - 1;
    $next = mysqli_query($link, "SELECT * FROM `categories` WHERE `id` =" . $next_id);
        if ($next) {
            $next = 1;
        } else {
            $next = 0;
        };
        $prev = mysqli_query($link, "SELECT * FROM `categories` WHERE `id` =" . $prev_id);
        if ($prev) {
            $prev = 1;
        } else {
            $prev = 0;
...
<div class="container">
        ...
        <div class="include">
            <div class="informathion-block">
                <div class="header__content-inner">
                    <div class="info3">  
                    <div class="statyc_cat">
                        <div class="stG_cat"> <center> <div class="st"><?= $row['genus'] ?><br> рода</div></center></div>
                        <div class="stG_cat"> <center><div class="st"><?= $row['type'] ?><br> видов</div></center></div>
                    </div>
                        <p class="padding"><h3 class="textG"><?= $row['classification'] ?></h3></p>
                        <p class="padding"><h3 class="textG"><?= $row['description'] ?></h3></p>
                    </div>
                    <img src="img/categories/<?= $row['img'] ?>" class="img2">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
    <div class="include">
            <center><h1>Представители</h1></center>
          
                <div id="categories">
                <?php foreach ($imgs as $img) { ?>
                    <div class="card">
                        <img class="categ_img_type" src="img/categories/members/<?= $img['img_slide'] ?>" alt="">
                        <h3 class="ad_slider_text"><?= $img['name'] ?></h3>
                    </div>
                <?php } ?>  
                </div>
    ...
    <div class="container">
        <div class="header__content-inner">
            ...
            <?php if ($prev) { ?>
                        <p class="left"> 
                            <a href="categories-item.php?id=<?= $prev_id ?>">
            ...
            <?php } if ($next) { ?>
                        <p class="right">
                            <a href="categories-item.php?id=<?= $next_id ?>">
                                <svg width="78" height="53" viewBox="0 0 78 53" fill="none" xmlns="http://www.w3.org/2000/svg">
